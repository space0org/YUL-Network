...
# DESCRIPTION
#
#   Check whether the given FLAG works with the current language's preprocessor
...
AS_VAR_PUSHDEF([CACHEVAR],[ax_cv_check_[]_AC_LANG_PREFIX[]cppflags_$4_$1])dnl
AC_CACHE_CHECK([whether _AC_LANG preprocessor accepts $1], CACHEVAR, [
  ax_check_save_flags=$[]_AC_LANG_PREFIX[]CPPFLAGS
  _AC_LANG_PREFIX[]CPPFLAGS="$[]_AC_LANG_PREFIX[]CPPFLAGS $4 $1"
  AC_PREPROC_IFELSE([m4_default([$5],[AC_LANG_PROGRAM()])],
    [AS_VAR_SET(CACHEVAR,[yes])],
    [AS_VAR_SET(CACHEVAR,[no])])
  _AC_LANG_PREFIX[]CPPFLAGS=$ax_check_save_flags])
